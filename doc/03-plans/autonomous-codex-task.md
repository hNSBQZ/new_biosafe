...
你拥有本机开发所需权限，但权限不扩大产品范围：不得修改旧仓库，不得泄露或提交密钥，不得删除/覆盖非本任务创建的 RAGFlow 资源。RAGFlow live 测试只能操作 `biosafe-dev-` 前缀资源。`demand.txt` 中 token 跨行，读取时必须去除说明文字且不可打印；旧仓库 `.env` 可作为本地服务配置来源但不可提交。
