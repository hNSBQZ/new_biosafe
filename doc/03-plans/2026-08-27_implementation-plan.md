...
- 实现历史分页、引用查看和纠错编辑。
- 做桌面/移动响应式和无障碍检查；Playwright 截图/关键流程检查留到 Phase 6。
- 写 Phase 4 记录并提交。
...
- `DONE`：建立内置 React 静态产物的单一生产 API 镜像、宿主端口映射、Compose 健康检查与 SQLite/日志命名卷，补齐宿主 Nginx、备份恢复和升级说明。
...
- `DONE`：将历史与人工纠错页收敛为“问题 + 日期”单列条目，展开后通过独立弹窗查看答案、模型复核和人工标注，保持人工显式保存边界。
...
- `DONE`：修复当前 RAGFlow 版本中文档名称过滤参数不兼容导致的同名预检查失败，补充 adapter 契约、管理 API 回归测试和真实实例只读验证。
- `DONE`：增加知识库“检索匹配”诊断页，严格复用在线 RAG 默认数据集与检索参数，展示 RAGFlow 返回的前序 chunks、来源和相似度。
