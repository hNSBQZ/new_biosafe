...
- 实际变更：新增单镜像 `Dockerfile`、`.dockerignore`、`docker-compose.yml`、`deploy/production.env.example` 和空实验目录；新增 `BIOSAFE_WEB_DIST_PATH`；FastAPI 在 API 路由之后提供静态资源和 SPA fallback，未知 `/api/*` 仍保持 JSON 404；新增生产配置差异、旧变量迁移、首次启动、健康检查、备份、恢复、升级和回滚文档。
- 数据或接口兼容性：无 SQLite schema 变更，无业务 API 变更。数据库固定位于 `BIOSAFE_DATA_VOLUME` 对应命名卷的 `/app/data/biosafe.db`，WAL/SHM 与主文件同卷。生产配置可由 `BIOSAFE_ENV_FILE` 指向仓库外任意文件，Compose 内容器路径覆盖不依赖开发机 `.env`。
